...
import { decodeHtmlEntities } from "../newsTextUtils.js";
 ...
 */
/**
 * HTML یکدست برای نمایش در UI — فقط از cleaned_text
 * @param {{ cleaned_text?: string, source_platform?: string }} row
 */
export function resolveDisplayHtml(row) {
  return ensureStoredCleanedHtml(
    row?.cleaned_text,
    null,
    row?.source_platform ?? SOURCE_PLATFORM.AUTO,
  );
}

/** نرمال‌سازی cleaned_text برای ذخیره در DB — همیشه HTML */
export function ensureStoredCleanedHtml(cleanedText, _rawText, sourcePlatform = SOURCE_PLATFORM.AUTO) {
  const cleaned = String(cleanedText ?? "").trim();
  if (!cleaned) return "";

  if (isLikelyHtml(cleaned)) return cleaned;

  return ingestRawToCleanedHtml(cleaned, sourcePlatform);
}
